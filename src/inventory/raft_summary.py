...
def run(config):
    ...
    for p in files:
        ...
        for name,st,en,kind,status,reason in df:
            migration = "A_ALREADY_SAFE" if status == "DSL_SAFE" else "E_UNSUPPORTED_OR_UNKNOWN"
            why = "Rust DSL function; no explicit unsafe detected" if status == "DSL_SAFE" else "Rust DSL function contains explicit unsafe; not counted as currently verified safe"
            records.append({"file":str(p.relative_to(repo)),"qualified_name":name,"start_line":st,"end_line":en,"source_kind":kind,"generated":False,"rustycpp_status":status,"migration_class":migration,"reason":why})
    ...
    dsl_count=sum(r["source_kind"]=="RUST_DSL" for r in records); explicit=sum(r["rustycpp_status"]=="DSL_EXPLICIT_UNSAFE" for r in records); unknown_dsl=sum(r["source_kind"]=="RUST_DSL" and r["rustycpp_status"]=="DSL_UNVERIFIED_OR_UNKNOWN" for r in records); cpp_unverified=sum(r["rustycpp_status"]=="CPP_UNVERIFIED" for r in records); boundaries=counts["D_BOUNDARY"]
    boundary_files={r["file"] for r in records if r["migration_class"]=="D_BOUNDARY"}
    result={"schema_version":1,"repository":meta(repo),"included_files":[str(p.relative_to(repo)) for p in files],"excluded_files":[{"file":f,"reason":r} for f,r in excluded],"functions_total_logical":total,"functions_verified_safe":safe,"current_verified_safe_function_pct":100*safe/total if total else 0,"dsl_function_count":dsl_count,"dsl_function_pct":100*dsl_count/total if total else 0,"dsl_safe_function_count":safe,"dsl_explicit_unsafe_function_count":explicit,"dsl_unverified_or_unknown_function_count":unknown_dsl,"cpp_verified_safe_function_count":0,"cpp_explicit_unsafe_function_count":0,"cpp_unverified_function_count":cpp_unverified,"unknown_function_count":unknown_dsl,"rust_dsl_nonblank_loc":dloc,"handwritten_cpp_nonblank_loc":cloc,"generated_cpp_nonblank_loc":gloc,"authored_nonblank_loc":authored,"dsl_authored_loc_pct":100*dloc/authored if authored else 0,"migration_counts":dict(counts),"boundary_functions":boundaries,"boundary_function_pct":100*boundaries/total if total else 0,"boundary_files":len(boundary_files),"boundary_file_pct":100*len(boundary_files)/len(files) if files else 0,"potential_safe_envelope_functions":counts["A_ALREADY_SAFE"]+counts["B_DIRECT_MIGRATION"]+counts["C_LOCAL_REFACTOR"],"direct_safe_envelope_functions":counts["A_ALREADY_SAFE"]+counts["B_DIRECT_MIGRATION"]}
